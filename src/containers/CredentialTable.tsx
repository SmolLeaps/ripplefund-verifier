...
const CredentialTable = () => {
  ...
  useEffect(() => {
    if (globalToken) {
      setCredentialShareResponseToken((prevState) => [
        ...prevState,
        globalToken,
      ]);
    }
  ...
  useEffect(() => {
    const onValidate = async (token: string) => {
      ...
      const credentialType =
        result.suppliedCredentials[0].type[
          result.suppliedCredentials[0].type.length - 1
        ];
      let drivingClass: string | undefined = undefined;
      if (credentialType === "IDDocumentCredentialPersonV1") {
        drivingClass = JSON.parse(
          result.suppliedCredentials[0].credentialSubject.data.hasIDDocument
            ?.hasIDDocument.idClass
        ).drivingClass;
      }

      setVCData((prevState) => [
        ...prevState,
        { token, validatedResult: result },
      ]);
    };
          ...
          {vcData.map((data, index) => {
            return (
              <tr>
                ...
                <td>
                  {
                    data.validatedResult.suppliedCredentials[0]
                      .credentialSubject.data.givenName
                  }
                </td>
                <td>
                  {data.validatedResult.isValid ? (
                    <img
                      src={CheckCircle}
                      alt="check"
                      style={{ height: "28px" }}
